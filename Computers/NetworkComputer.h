...
#ifndef NETWORK_COMPUTER_H

#define NETWORK_COMPUTER_H
...
template <int Dim>
class CNetworkComputer : public CBaseComputer<Dim>
{
private:
	typedef Eigen::Matrix<dbl,Dim,1>   dvec;
	typedef Eigen::Matrix<dbl,Dim,Dim> dmat;
	CNetworkState<Dim> &State;
...
public:
	...
	CNetworkComputer();
	CNetworkComputer(CNetworkState<Dim> &_State);
	CNetworkComputer(const CNetworkComputer<Dim> &_Copy);
	
		
//get and set the state
	CNetworkState<Dim> &GetState() const;
	void SetState(CNetworkState<Dim> & State);
...
template <int Dim>
CNetworkComputer<Dim>::CNetworkComputer() : FixDof(false)
{
}


template <int Dim>
CNetworkComputer<Dim>::CNetworkComputer(CNetworkState<Dim> &_State) : State(_State), FixDof(false)
{	
}

template <int Dim>
CNetworkComputer<Dim>::CNetworkComputer(const CNetworkComputer<Dim> &_Copy) : State(_Copy.State), FixDof(_Copy.FixDof), FixedDof(_Copy.FixedDof)
{
}
...
//get and set the state
template <int Dim>
CNetworkState<Dim> &CNetworkComputer<Dim>::GetState() const
{
	return State;
}

template <int Dim>
void CNetworkComputer<Dim>::SetState(CNetworkState<Dim> & _State)
{
	State = _State;
}
	
template <int Dim>
inline dbl CNetworkComputer<Dim>::GetVolume() const
{
	return State.GetVolume();
}

template <int Dim>
inline int CNetworkComputer<Dim>::GetNdof() const
{
	return State.GetNodeNumber()*Dim;
}

template <int Dim>
inline dbl CNetworkComputer<Dim>::GetMinimizationTimeScale() const
{
	return State.GetAverageELength()/2.; //This is emperical!!! NOT GENERAL!
}
...
void CNetworkComputer<Dim>::ComputeBondList(CBondList<Dim> &bonds)
{
	...
	dvec Displacement;
	bonds.SetN(State.GetNodeNumber());
	
	dbl dr, dr0, k0, E, g, k;
	int i, j;
	for(int bi=0; bi<State.GetNBonds(); ++bi)
	{
		State.GetBondInfo(bi, i, j, dr0, k0); //Get i, j, dr0, k0
		State.GetDisplacement(i,j,Displacement);
		dr = Displacement.norm();
		State.GetPotential()->ComputeDerivatives012(dr, dr0, k0, E, g, k); //Compute E, g, k
		bonds.AddBond( CBond<Dim>(i, j, dr, E, g, k, Displacement) );
	}
 ...
 */
template <int Dim>
int CNetworkComputer<Dim>::StdPrepareSystem(bool verbose)
{
	std::vector<bool> fixed; fixed.assign(State.GetNodeNumber(),false);
 ...
 */
template <int Dim>
int CNetworkComputer<Dim>::StdPrepareSystem(vector<bool> const &fixed, bool verbose)
{
...
template <int Dim>
void CNetworkComputer<Dim>::CalculateStdData(bool CalcCijkl, bool CalcHess)
{
	CalculateStdData(Data,CalcCijkl,CalcHess);
}
template <int Dim>
void CNetworkComputer<Dim>::CalculateStdData(CStdData<Dim> &data, bool CalcCijkl, bool CalcHess)
{
...
template <int Dim>
void CNetworkComputer<Dim>::CalculateStdData_Unstressed(bool CalcCijkl, bool CalcHess)
{
...
template <int Dim>
void CNetworkComputer<Dim>::CalculateStdData_Unstressed(CStdData<Dim> &data, bool CalcCijkl, bool CalcHess)
{
...
template <int Dim>
void CNetworkComputer<Dim>::SetFixedDof(vector<bool> const &_FixedDof)
{
	FixDof = true;
	FixedDof = _FixedDof;
...
template <int Dim>
void CNetworkComputer<Dim>::UnsetFixedDof()
{
	FixDof = false;
}

template <int Dim>
void CNetworkComputer<Dim>::Evaluate(Eigen::VectorXd &grad, dbl &fx) 
{
	CBondList<Dim> bonds;
	ComputeBondList(bonds);
...
template <int Dim>
bool CNetworkComputer<Dim>::Progress(Eigen::VectorXd const &grad, dbl fx, int iteration, int print_iter, dbl tol) 
{
...
template <int Dim>
bool CNetworkComputer<Dim>::ProgressSilent(Eigen::VectorXd const &grad, dbl fx, int iteration, int print_iter, dbl tol) 
{
	dbl gradNorm = grad.norm();
	dbl max_grad = max_abs_element(grad.size(), grad.data());
	bool converged = (max_grad < tol)?true:false;
...
template <int Dim>
inline void CNetworkComputer<Dim>::Move(Eigen::VectorXd const &step) 
{
	State.MoveNodes(step);
};
...
template <int Dim>
void CNetworkComputer<Dim>::ComputeHessian(Eigen::SparseMatrix<dbl> &hess) const
{
	ComputeHessian(Bonds, hess);
}

template <int Dim>
void CNetworkComputer<Dim>::ComputeHessian(CBondList<Dim> const &bonds, Eigen::SparseMatrix<dbl> &hess) const
{
	bonds.ComputeHessian(hess);
}

template <int Dim>
void CNetworkComputer<Dim>::ComputeHessianBZ(Eigen::SparseMatrix<cdbl> &hess, Eigen::SparseMatrix<cdbl> &transformation, dvec const &k) const
{
	ComputeHessianBZ(Bonds, hess, transformation, k);
}
template <int Dim>
void CNetworkComputer<Dim>::ComputeHessianBZ(CBondList<Dim> const &bonds, Eigen::SparseMatrix<cdbl> &hess, Eigen::SparseMatrix<cdbl> &transformation, dvec const &k) const
{
...
template <int Dim>
void CNetworkComputer<Dim>::CalculateBZTransformation(Eigen::SparseMatrix<cdbl> &transformation, dvec const &k) const
{
	CalculateBZTransformation(Bonds, transformation, k);
}

template <int Dim>
void CNetworkComputer<Dim>::CalculateBZTransformation(CBondList<Dim> const &bonds, Eigen::SparseMatrix<cdbl> &transformation, dvec const &k) const
{
...
template <int Dim>
dbl CNetworkComputer<Dim>::ComputeEnergy()
{
	Grid.Construct();
...
template <int Dim>
void CNetworkComputer<Dim>::ComputeGradient(Eigen::VectorXd &tar)
{
	Grid.Construct();
...
template <int Dim>
void CNetworkComputer<Dim>::ComputeForce(Eigen::VectorXd &tar)
{
	ComputeGradient(tar);
	tar = -tar;
}
...
//Hessian stuff
template <int Dim>
void CNetworkComputer<Dim>::ComputeHessian(Eigen::MatrixXd &tar) 
{
	Grid.Construct();
...
//computes at q = 0
template <int Dim>
void CNetworkComputer<Dim>::ComputeDynamicalMatrix(Eigen::MatrixXd &tar)
{
	Grid.Construct();
