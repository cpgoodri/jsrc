...
class CSoftSpotCalculator
{
	...
	static void Decompose(int Nvar, dbl *vecs, int Nm, int Np, vector<bool> &softparticles);
...
template <int Dim>
CSoftSpotComputer<Dim>::CSoftSpotComputer(CStaticState<Dim> &_State) : State(_State), Grid(&State), FixDof(false), UseGrid(true)
{
        //use grid or not
        dvec MaxDistance;
