...
#ifndef KALJ_POTENTIAL
#define KALJ_POTENTIAL
...
class CKALJPotential : public CPotential
{	
private:
...
///@{
	CKALJPotential();
	CKALJPotential(dbl _e);
	CKALJPotential(const CKALJPotential &box);
	const CKALJPotential &operator=(const CKALJPotential &box);
///@}
...
CKALJPotential::CKALJPotential()
{
	epsilon = 1.0;
}

CKALJPotential::CKALJPotential(dbl _e)
{
	epsilon = _e;
}

CKALJPotential::CKALJPotential(const CKALJPotential &pot) : epsilon(pot.epsilon), CPotential(pot) //CPG comment: is is necessary to call the CPotential constructor? If so, then this should be done in CHarmonicPotential as well.
{
}
	
const CKALJPotential &CKALJPotential::operator=(const CKALJPotential &pot)
{
	epsilon = pot.epsilon;
...
string CKALJPotential::GetName()
{
	string s = "KALJPotential";
	return s;
}

//functions to write potential configurations
string CKALJPotential::DataToString() const
{
	stringstream ss; 
...
void CKALJPotential::StringToData(string Data)
{
...
CKALJPotential* CKALJPotential::Clone() const
{
        return new CKALJPotential( *this );
} 
	
//functions to compute various derivatives
dbl CKALJPotential::Compute(dbl dr,dbl r) const
{
...
dbl CKALJPotential::ComputeFirstDerivative(dbl dr,dbl r) const
{
	dbl epsij=epsilon;
        if (r < 0.84) epsij = 1.5*epsilon;
        if (r > 0.84 && r<0.9) epsij = 0.5* epsilon;
...
dbl CKALJPotential::ComputeSecondDerivative(dbl dr,dbl r) const
{	
	dbl epsij=epsilon;
        if (r < 0.84) epsij = 1.5*epsilon;
        if (r > 0.84 && r<0.9) epsij = 0.5* epsilon;
...
dbl CKALJPotential::ComputeThirdDerivative(dbl dr,dbl r) const
{
	dbl epsij=epsilon;
        if (r < 0.84) epsij = 1.5*epsilon;
        if (r > 0.84 && r<0.9) epsij = 0.5* epsilon;
...
void CKALJPotential::ComputeDerivatives01(dbl dr, dbl r, dbl &E, dbl &g) const
{
	dbl epsij=epsilon;
        if (r < 0.84) epsij = 1.5*epsilon;
        if (r > 0.84 && r<0.9) epsij = 0.5* epsilon;
...
void CKALJPotential::ComputeDerivatives012(dbl dr, dbl r, dbl &E, dbl &g, dbl &k) const
{
	dbl epsij=epsilon;
        if (r < 0.84) epsij = 1.5*epsilon;
        if (r > 0.84 && r<0.9) epsij = 0.5* epsilon;
...
void CKALJPotential::ComputeDerivatives0123(dbl dr, dbl r, dbl &E, dbl &g, dbl &k, dbl &t) const
{
	dbl epsij=epsilon;
        if (r < 0.84) epsij = 1.5*epsilon;
        if (r > 0.84 && r<0.9) epsij = 0.5* epsilon;
...
dbl CKALJPotential::ComputeSupport() const
{
	return 2.5;
}

bool CKALJPotential::Overlapping(dbl rad1, dbl rad2, dbl rlen2, dbl &sigma) const
{	
